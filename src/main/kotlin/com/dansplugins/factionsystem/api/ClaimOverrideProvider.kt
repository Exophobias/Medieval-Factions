 ...
 *
 * ## Refuse [ClaimAction.CONTAINER]
 *
 * Strongly advised, and the reason [ClaimAction] exists at all. MF routes inventory clicks through
 * the same decision as block breaks, so a provider that ignores the action and returns true would
 * hand out chest access along with the right to place a block. Almost no legitimate use case wants
 * that, and the ones that think they do usually want a dedicated container instead.
 *
 * ## Failure is contained
 *
 * A provider that throws is caught, logged once, and treated as "no opinion". A broken third-party
 * plugin must not be able to break protection for the whole server in either direction.
 *
...
fun interface ClaimOverrideProvider {
     ...
     * @param y block Y
     * @param z block Z
     * @param action what is being attempted; never [ClaimAction.UNKNOWN]
